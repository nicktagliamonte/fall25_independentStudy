...
func runRun(args []string) error {
	...
	// Merge extra seed sources (CLI --seed, SNG40_SEEDS env, --seed-file)
	// into a single list, as buildNodeSubsystems expects.
	extraSeeds := append([]string{}, seedAddrs...)
	if env := os.Getenv("SNG40_SEEDS"); env != "" {
		for _, s := range strings.Split(env, ",") {
			s = strings.TrimSpace(s)
			if s != "" {
				extraSeeds = append(extraSeeds, s)
			}
		}
	}
	if seedFile != "" {
		if b, err := os.ReadFile(seedFile); err == nil {
			for _, line := range strings.Split(string(b), "\n") {
				...
				if line != "" && !strings.HasPrefix(line, "#") {
					extraSeeds = append(extraSeeds, line)
				}
			}
		}
	}

	staleAge, staleAgeErr := time.ParseDuration(staleAgeStr)

	subs, err := buildNodeSubsystems(ctx, nodeSubsystemsConfig{
		KeyPath:          keyPath,
		ListenMultiaddrs: listenAddrs,
		StorePath:        storePath,
		BootstrapPeers:   extraSeeds,
		HasPeerPolicy:    staleAgeErr == nil,
		StaleAge:         staleAge,
		MaxFailures:      maxFailures,
		MaxKnown:         maxKnown,
		MinOutbound:      minOutbound,
		ClusterNodeCount: clusterNodes,
		TSHAddr:          os.Getenv("TSH_ADDR"),
		Cancel:           cancelMain,
	})
	if err != nil {
		return err
	...
	stack := subs.Stack
	dht := subs.DHT
	dynamicRouter := subs.DynamicRouter
	peerStore := subs.PeerStore
	metrics := subs.Metrics
	policyBase := subs.PolicyBase
	repairProtocol := subs.RepairProtocol
	gateway := subs.Gateway
	headStr := subs.HeadStr
	height := subs.Height
	...
	startDialMaintenanceLoop(ctx, h, peerStore, policyBase, metrics, dialLoopConfig{
		MinOutbound:      minOutbound,
		ClusterNodeCount: clusterNodes,
		PerIPDialLimit:   perIPDialLimit,
	...
	startGossipLoop(ctx, h, policyBase, func(pid peer.ID, res *myhost.HandshakeResult) {
		_, _, _, _ = mystore.AppendPeerAddedIfNew(context.Background(), stack.Datastore, stack.BlockSvc, pid.String())
...
// runPut implements the "put" subcommand.
...
// runShutdown implements the "shutdown" subcommand.
...
// runSnapshot implements the "snapshot" subcommand.
...
// runNeighbors implements the "neighbors" subcommand.
...
// runLookupKeyCmd implements the "lookup-key" subcommand.
// "lookup-key" performs a one-off, stateless DHT lookup from a fresh node.
func runLookupKeyCmd(args []string) error {
