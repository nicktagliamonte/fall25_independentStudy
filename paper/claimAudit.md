...
| Prefix queries use sharded PHTs in production | shard stores, per-shard ownership keys, incremental PHT, production constructors, live DHT record test, and direct fanout/node-fetch counters | Implemented and instrumented | Run catalog-size/selectivity experiment |
| Substring queries use Bloom pruning in production | live Bloom maintenance, parallel shard queries, indexed substring path, multi-peer integration test, and branch/pruning counters | Implemented and instrumented | Add pruning-disabled ablation mode and run ablation |
| Index mutation authority is distributed | full-name hash selects one of 16 shards; each shard resolves a distinct overlay owner; per-shard/local/remote/failure/service-time counters | Implemented and instrumented | Run mutation throughput and owner-distribution experiment as shard count varies |
