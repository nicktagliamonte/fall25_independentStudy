...
# Usage: ./scripts/scenarios/swarm_download_test.sh [options]
...
echo "system,payload_size,iteration,ttfb_ms,total_ms" > "$OUTPUT_FILE"
...
download_our_system() {
  ...
  # Copy request to container and execute curl inside container
  docker cp "$get_req" "${OUR_CONTAINER}:/tmp/get_req_$$.json" >/dev/null 2>&1
  
  # Use curl with write-out to measure TTFB
  local curl_output=$(docker exec "$OUR_CONTAINER" curl -sSf -w "\n%{time_starttransfer}\n%{time_total}" -X POST \
    -H "Content-Type: application/json" \
    -d @/tmp/get_req_$$.json \
    "http://$OUR_API_ADDR/get" 2>&1)
  
  docker exec "$OUR_CONTAINER" rm -f "/tmp/get_req_$$.json" >/dev/null 2>&1 || true
  rm -f "$get_req"
  ...
  if [[ -n "$OUR_CID" ]]; then
    for i in $(seq 1 $ITERATIONS); do
      ...
      if [[ $? -eq 0 ]]; then
        ...
        echo "our_system,$size,$i,$ttfb,$total" >> "$OUTPUT_FILE"
      else
        ((our_failures++))
        echo "FAILED"
        echo "our_system,$size,$i,ERROR,ERROR" >> "$OUTPUT_FILE"
      fi
    done
  else
  ...
  if [[ -n "$SWARM_HASH" ]]; then
    for i in $(seq 1 $ITERATIONS); do
      ...
      if [[ $? -eq 0 ]]; then
        ...
        swarm_total+=("$total")
        echo "TTFB: $(printf "%.2f" "$ttfb") ms, Total: $(printf "%.2f" "$total") ms"
        echo "swarm,$size,$i,$ttfb,$total" >> "$OUTPUT_FILE"
      else
        ((swarm_failures++))
        echo "FAILED"
        echo "swarm,$size,$i,ERROR,ERROR" >> "$OUTPUT_FILE"
      fi
    done
  else
    echo "    Skipping (upload failed)"
