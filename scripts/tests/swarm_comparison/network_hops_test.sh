...
put_req() {
  ...
  local resp=$(docker exec "$OUR_CONTAINER" curl -sSf -X POST -H "Content-Type: application/json" \
    -d @/tmp/put_$$.json "http://$OUR_API_ADDR/put" 2>&1)
...
get_req() {
  ...
  docker cp "$json" "${OUR_CONTAINER}:/tmp/get_$$.json" >/dev/null 2>&1
  local resp=$(docker exec "$OUR_CONTAINER" curl -sSf -X POST -H "Content-Type: application/json" \
    -d @/tmp/get_$$.json "http://$OUR_API_ADDR/get" 2>&1)
  docker exec "$OUR_CONTAINER" rm -f /tmp/get_$$.json >/dev/null 2>&1 || true
  echo "$resp"
}
